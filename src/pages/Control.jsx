...
export default function Control() {
  const { activeState: d, actions } = useApp();
    ...
    <div className={s.page}>
      ...
      <div className={`card ${s.eaCard}`}>
        <div className={s.eaTop}>
          ...
          <button
            className={`${s.eaBtn} ${running?s.pauseBtn:s.resumeBtn}`}
            onClick={async () => {
              await actions.command(running ? 'pause' : 'resume');
            }}>
      ...
      {[
        ...
        <div key={t.sw} className={`card ${s.toggleCard}`}>
          ...
          <div className={`sw ${t.on?'on':''}`}
               onClick={() => actions.command(t.on ? t.off_act : t.on_act)} />
        </div>
      ))}
...
function getParamVal(d, key) {
  const map = {
    risk: d.riskPercent+'%',  tp: d.tpPoints+' pts',
    barsn: d.barsN+' bars',   starthour: d.startHour===0?'24h (off)':d.startHour+'h',
  };
  return map[key] || '—';
}
